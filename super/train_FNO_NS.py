...
# ----------------------------
logging.basicConfig(
    filename='fno_training_NS_nu=1e-4_mode=7.5.log',
    level=logging.INFO,
...
def load_fine_data_both():
    ...
    fine_data = np.load(f'/pscratch/sd/h/hbassi/NavierStokes_fine_256_nu0.0001_k7.5.npy')
    # inputs, targets = [], []
    # for traj in fine_data:
    #     inputs.append(traj[:100])   
    #     targets.append(traj[1:101]) 
    inputs = torch.tensor(fine_data[:, :100], dtype=torch.float32)
    targets = torch.tensor(fine_data[:, 101], dtype=torch.float32).unsqueeze(1)
...
def train_finetune():
    ...
    model_u = FNO2d(100, 1, 16, 16, 64).to(device)
    
    optimizer = optim.AdamW(list(model_u.parameters()), lr=0.01)
    ...
    for epoch in trange(num_epochs):
        ...
        if avg_val_loss < best_val_loss:
            ...
            torch.save(model_u.state_dict(), f"/pscratch/sd/h/hbassi/FNO_fine_tuning_NS_64to256_nu={1e-4}_mode={7.5}_best_model.pth")
