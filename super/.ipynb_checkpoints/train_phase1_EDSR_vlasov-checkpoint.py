...
def load_data():
    ...
    input_CG = np.load(f'/pscratch/sd/h/hbassi/2d_vlasov_multi_traj_coarse_32_data.npy')[:, :]
    target_FG = np.load(f'/pscratch/sd/h/hbassi/2d_vlasov_multi_traj_fine_128_data.npy')[:, :]
...
def train_model():
    ...
    #val_indices = [400, 793, 1194]
    #train_indices = list(set(range(1195)) - set(val_indices))
    ...
    model = EDSR(11, 128, 4, 4, data_mean, data_std).to(device)
    ...
    for epoch in trange(num_epochs + 1):
        ...
        if epoch % 100 == 0:
            ...
            }, f"/pscratch/sd/h/hbassi/models/EDSR_2d_vlasov_multi_traj_checkpoint_epoch_{epoch}_RK_32to128.pth")
            
            # Save best model
            if avg_val_loss < best_val_loss:
                best_val_loss = avg_val_loss
                torch.save(model.state_dict(), f"/pscratch/sd/h/hbassi/models/EDSR_2d_vlasov_multi_traj_best_model_RK_32to128.pth")
                print(f"New best model saved with val loss {avg_val_loss:.8f}")
        with open(f'./logs/EDSR_2d_vlasov_multi_traj_training_RK_32to128.npy', 'wb') as f:
            np.save(f, training_losses)
        f.close()
        with open(f'./logs/EDSR_2d_vlasov_multi_traj_validation_RK_32to128.npy', 'wb') as f:
