...
STATS_TPL = "./data/stats_{tag}.pt"
TAG       = "nu1e-4_k7.5"        # adjust if needed
...
def main(args):
    ...
    # ---------- load mean / std (force fp32) ----------
    stats = torch.load(STATS_TPL.format(tag=TAG), map_location="cpu")
    mean  = stats["mean"].float().to(dev)       # (1,100,1,1)  float32
    std   = stats["std"] .float().to(dev)

    # ---------- input tensor (fp32) ----------
    x_in = torch.tensor(coarse[idx, :100], dtype=torch.float32,
                        device=dev).unsqueeze(0)              # (1,100,64,64)
    ...
    for m in MODELS:
        ...
        net = (
            ...
            models.EDSR(100, 128, 16, 4, np.zeros(100,dtype=np.float32), np.ones(100,dtype=np.float32))     if m == "edsr" else
            models.SuperResUNet(in_channels=100, final_scale=4)                     # FUNet
        )
        net.load_state_dict(ckpt)
        net.to(dev).eval()                          # move, but keep fp32 / c64
        nets[m] = net
    ...
    # helper ───────────────────────────────────────────────────────
    def add_ticks(ax, N=256, show_y=False):
        xt = [0, N//2, N-1]
        ax.set_xticks(xt)
    ...
    # number of image panels = GT + Bicubic + |MODELS|
    n_img = 2 + len(MODELS)
    ...
    for t in frames:
        ...
        images = {
            "GT_COARSE": dict(img=gt_coarse, vmin=None, vmax=None, N=64,  show_y=True),
            "GT_FINE":   dict(img=gt_fine,   vmin=vmin, vmax=vmax, N=256, show_y=False),
            "UPS":       dict(img=up,        vmin=None, vmax=None, N=256, show_y=False),
            "FUNET":     dict(img=pred_bank["funet"][t], vmin=vmin, vmax=vmax, N=256, show_y=False),
            "EDSR":      dict(img=pred_bank["edsr"][t],  vmin=vmin, vmax=vmax, N=256, show_y=False),
            "FNO":       dict(img=pred_bank["fno"][t],   vmin=vmin, vmax=vmax, N=256, show_y=False),
            "UNET":      dict(img=pred_bank["unet"][t],  vmin=vmin, vmax=vmax, N=256, show_y=False),
        }
        ...
        for j, key in enumerate(COLS):
            ...
            # relative L2 w.r.t. GT-Fine (skip GT-Coarse title overlay)
            if key != "GT_COARSE":
                if key == "GT_FINE":
                    continue
            ...
            # remember GT-Fine handle for shared colorbar
            if key == "GT_FINE":
                im_ref = im
    
        # shared colorbar (far right) for GT-Fine scale
        #cax = fig.add_subplot(gs[0, -1])
        #cb  = fig.colorbar(im_ref, cax=cax)
        #cb.ax.tick_params(labelsize=8)
    
        fig.savefig(Path(args.out_dir) / f"vis_t{t:03d}.pdf",
    ...
    # 1) Upsampled (bicubic) and neural models first 
    k, E = radial_energy_spectrum(up)
    ...
    fig.savefig(Path(args.out_dir) / "energy_spectra.pdf",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
...
if __name__ == "__main__":
    ap = argparse.ArgumentParser("Evaluate Phase‑1 models on NS test set")
    ap.add_argument("--coarse", default="/pscratch/sd/h/hbassi/"
                    "NavierStokes_coarse_64_nu0.0001_k7.5_test_data.npy")
    ap.add_argument("--fine",   default="/pscratch/sd/h/hbassi/"
                    "NavierStokes_fine_256_nu0.0001_k7.5_test_data.npy")
    ap.add_argument("--index",  type=int, default=0,
                    help="which test trajectory to visualise")
    ap.add_argument("--out_dir", default="./NS_eval_figs", type=str)
    main(ap.parse_args())
